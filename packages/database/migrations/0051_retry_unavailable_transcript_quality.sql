-- Re-open completed transcription jobs whose post-STT quality assessment could
-- not satisfy the model/evidence contract. The transcript and segments already
-- exist, so the worker resumes from the quality step without submitting audio
-- to Speech-to-Text again.
WITH recoverable AS (
