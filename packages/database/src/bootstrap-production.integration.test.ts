...
describe.skipIf(!databaseUrl)("production database bootstrap",()=>{
  it("is dry-run by default, idempotent on apply, hash-only for email, and fail-closed on drift",async()=>{
    ...
    try{
      ...
      expect(dryRun.created).toHaveLength(26);
      ...
      expect(applied.created).toHaveLength(26);
      ...
      expect(repeated.existing).toHaveLength(26);
      ...
      expect(reappliedAfterOperationalChange.existing).toHaveLength(26);
      ...
      expect(dryRunAfterOperationalChange.existing).toHaveLength(26);
      expect((await pool.query<{enabled:boolean;owner_membership_id:string;rollback_note:string}>("SELECT enabled,owner_membership_id,rollback_note FROM feature_flags WHERE organization_id=$1 AND flag_key='team_analytics'",[organizationId])).rows[0]).toEqual({enabled:true,owner_membership_id:operationalMembershipId,rollback_note:"運用判断による継続"});
