...
describe("Vertex AI review parity contract",()=>{
  it("requires exactly the six review areas and the PoC rubric",()=>{const schema=vertexReviewOutputSchema() as {properties:{findings:{minItems:number;maxItems:number;items:{properties:{category:{enum:string[]}}}}}};expect(schema.properties.findings.minItems).toBe(6);expect(schema.properties.findings.maxItems).toBe(6);expect(schema.properties.findings.items.properties.category.enum).toEqual(["strength","improvement","talk","compliance","next_action","revisit"]);const prompt=vertexReviewPrompt({transcript:"",segments:[{id:"E0001",text:"来週またお願いします"}],criteria:{contract:"poc_review_parity_v1"}});expect(prompt).toContain("次回合意あり・決裁者不在・追加品の自己言及");expect(prompt).toContain("愛着保留・比較検討中・葛藤保留");expect(prompt).toContain("社交辞令は高シグナルに含めません");expect(prompt).toContain("告知・クーリングオフ・書面交付・押し買い");expect(prompt).toContain('"id":"E0001"');});
  it("restricts the schema and prompt to the selected review areas",()=>{const dimensions=["strength","compliance"] as const;const schema=vertexReviewOutputSchema(dimensions) as {properties:{findings:{minItems:number;maxItems:number;items:{properties:{category:{enum:string[]}}}}}};expect(schema.properties.findings).toMatchObject({minItems:2,maxItems:2});expect(schema.properties.findings.items.properties.category.enum).toEqual(dimensions);const prompt=vertexReviewPrompt({transcript:"",segments:[{id:"E0001",text:"説明しました"}],dimensions:[...dimensions]});expect(prompt).toContain("strength・compliance");expect(prompt).toContain("合計2件");expect(prompt).toContain("選択されていない観点は返さない");});
