...
describe("long review chunking",()=>{
  ...
  it("aggregates six categories while retaining original evidence ids",()=>{const categories=["strength","improvement","talk","compliance","next_action","revisit"];const result=(suffix:string)=>({model:"gemini-2.5-flash",summary:`summary-${suffix}`,findings:categories.map(category=>({category,title:`${category}-${suffix}`,description:category==="compliance"?"告知: ✅ 済\nクーリングオフ: ⚠️ 要確認\n書面交付: ❌ 未確認\n押し買い: ✅ 問題なし":`description-${suffix}`,recommendedAction:null,evidenceSegmentIds:[`segment-${suffix}`]}))});const combined=aggregateReviewChunks([result("1"),result("2")]);expect(combined.findings).toHaveLength(6);expect(combined.findings[0]?.evidenceSegmentIds).toEqual(["segment-1","segment-2"]);expect(combined.summary).toContain("summary-2");});
  it("aggregates only the requested review dimensions",()=>{const result={model:"gemini-2.5-flash",summary:"summary",findings:[{category:"strength",title:"良かった点",description:"説明",recommendedAction:null,evidenceSegmentIds:["segment-1"]},{category:"compliance",title:"法令",description:"告知: ✅ 済\nクーリングオフ: ✅ 済\n書面交付: ✅ 済\n押し買い: ✅ 問題なし",recommendedAction:null,evidenceSegmentIds:["segment-1"]}]};expect(aggregateReviewChunks([result],["strength","compliance"]).findings.map(finding=>finding.category)).toEqual(["strength","compliance"]);});
  ...
  it("rejects an incomplete compliance aggregation instead of fabricating an unavailable result",()=>{const result={model:"model",summary:"summary",findings:[{category:"compliance",title:"法令",description:"法令面を確認しました",recommendedAction:null,evidenceSegmentIds:["s1"]}]};expect(()=>aggregateReviewChunks([result],["compliance"])).toThrow("review compliance item missing: 告知");});
});
