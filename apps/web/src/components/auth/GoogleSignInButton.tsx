...
export function GoogleSignInButton({onSuccess,onError}:{onSuccess:()=>void;onError:(message:string)=>void}){
  ...
  async function login(){
    ...
    try{
      ...
      setWorking(false);
      onError("Googleログインを開始できませんでした。もう一度お試しください。");
    }
  }

  return configured
    ? <button type="button" aria-label="Googleでログイン" aria-busy={working} disabled={working} onClick={()=>void login()}>{working?"Googleログインへ移動しています…":"Googleでログイン"}</button>
