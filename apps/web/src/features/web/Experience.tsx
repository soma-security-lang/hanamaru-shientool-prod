...
import {
  AlertTriangle, ArrowRight, BookOpen, Check, ChevronLeft, ChevronRight,
...
const listDetailViews=["list","detail"] as const;
const sourceFieldViews=["source","fields"] as const;
const evidenceAnalysisViews=["analysis","evidence"] as const;
const scenarioViews=["scenario","chat","feedback"] as const;
const knowledgeViews=["filters","list","detail"] as const;
const recordingViews=["recording","transcript"] as const;
const reviewInputViews=["conversation","criteria"] as const;
const homeViews=["assistant","evidence","actions"] as const;
const preparationViews=["summary","checks","guidance"] as const;
const adminTripleViews=["list","edit","preview"] as const;
const analyticsViews=["chart","table"] as const;
...
function useMobileView<T extends string>(allowed:readonly T[],fallback:T){
  ...
  const value=allowed.includes(fromUrl as T)?fromUrl as T:fallback;
  function update(next:T,extras:Record<string,string|undefined>={},mode:"push"|"replace"="push"){
    const updated=new URLSearchParams(params.toString());updated.set("view",next);
    for(const [key,item] of Object.entries(extras)){if(item)updated.set(key,item);else updated.delete(key);}
    router[mode](`${pathname}?${updated.toString()}`,{scroll:false});
  }
  return[value,update] as const;
}

function MobilePaneTabs<T extends string>({value,onChange,items,label}:{value:T;onChange:(value:T)=>void;items:ReadonlyArray<{value:T;label:string;disabled?:boolean}>;label:string}){
  return <nav className={styles.mobilePaneTabs} aria-label={label}>{items.map(item=><button type="button" aria-current={value===item.value?"page":undefined} disabled={item.disabled} onClick={()=>onChange(item.value)} key={item.value}>{item.label}</button>)}</nav>;
}

function MobilePaneBack({children,onClick}:{children:string;onClick:()=>void}){return <button type="button" className={styles.mobilePaneBack} onClick={onClick}><ChevronLeft size={18}/>{children}</button>;}
...
function PilotContentNotice({enabled,policy}:{enabled:boolean;policy?:ContentPolicyDto}){
  ...
  return <div className={styles.pilotNotice} role="status"><AlertTriangle size={20} aria-hidden="true"/><div><strong>未承認コンテンツ使用・要確認</strong><p>{policy?.notice??"限定運用では、移行内容が未承認のコンテンツを使用する場合があります。回答や提案は原文と照合して判断してください。"}</p></div></div>;
}
...
function visitFieldLabel(key:string){return({visitDate:"訪問予定日",visitTime:"訪問予定時間",visitDateTime:"訪問予定日時",scheduledAt:"訪問予定日時",customerLabel:"お客様表示名",customerName:"お客様表示名",appraisalItems:"査定品",visitAddress:"住所",contact:"連絡先",parking:"駐車場",campaign:"キャンペーン",notes:"備考",assignedStaffName:"担当"}[key]??key);}
...
function AiHome({pilotContentAi}:{pilotContentAi:boolean}) {
  ...
  const [mobileView,setMobileView]=useMobileView(homeViews,"assistant");
  ...
  return <>
    ...
    <MobilePaneTabs value={mobileView} onChange={setMobileView} label="ホームの表示" items={[{value:"assistant",label:"AIに相談"},{value:"evidence",label:"回答の根拠",disabled:!evidence.length},{value:"actions",label:"業務メニュー"}]}/>
    <div className={styles.homeGrid} data-mobile-view={mobileView}>
...
function VisitList() {
  ...
  const filtered=(remote.value??[]).filter(visit=>statusFilter==="all"||visit.status===statusFilter);const [selectedId,setSelectedId]=useState<string|undefined>(()=>params.get("selected")??undefined);const [mobileView,setMobileView]=useMobileView(listDetailViews,"list");const selected=filtered.find(visit=>visit.resourceId===selectedId)??filtered[0];
  return <>
    ...
    <div className={styles.listDetail} data-mobile-view={mobileView}>
      <section className={styles.tablePane} aria-label="訪問一覧">{remote.error?<p role="alert">訪問一覧を読み込めませんでした。時間をおいて再読み込みしてください。</p>:null}<table><thead><tr><th>訪問日時</th><th>案件番号</th><th>店舗</th><th>状態</th><th>次の作業</th></tr></thead><tbody>{filtered.map(visit=><tr data-selected={selected===visit} key={visit.resourceId}><td data-label="訪問日時">{visit.time}</td><td data-label="案件番号"><button className={styles.tableSelectButton} aria-pressed={selected===visit} onClick={()=>{setSelectedId(visit.resourceId);setMobileView("detail",{selected:visit.resourceId});}}><strong>{visit.id}</strong></button></td><td data-label="店舗">{visit.branch}</td><td data-label="状態"><Status>{visitStateLabel(visit.status).label}</Status></td><td data-label="次の作業">{visit.next}</td></tr>)}</tbody></table></section>
      {selected?<aside className={styles.detailPane}><MobilePaneBack onClick={()=>setMobileView("list")}>訪問一覧へ戻る</MobilePaneBack><span className={styles.kicker}>選択中の訪問</span><h2>{selected.id}</h2><dl><div><dt>訪問日時</dt><dd>{selected.time}</dd></div><div><dt>担当店舗</dt><dd>{selected.branch}</dd></div><div><dt>現在の状態</dt><dd>{visitStateLabel(selected.status).label}</dd></div></dl><Link className={styles.primaryButton} href={selected.href}>{selected.next}<ArrowRight size={17} /></Link><Link className={styles.textButton} href={`/visits/${selected.resourceId}/import`}>訪問情報を確認</Link></aside>:<aside className={styles.detailPane}><MobilePaneBack onClick={()=>setMobileView("list")}>訪問一覧へ戻る</MobilePaneBack><h2>訪問はありません</h2><p>PDFから訪問を登録すると、準備状況がここに表示されます。</p></aside>}
    </div>
  </>;
}

function VisitNavigation({ current,id }: { current: string;id:string }) {
  const steps=visitSteps(id);const currentIndex=Math.max(0,steps.findIndex(([label])=>label===current));
  return <div className={styles.visitNavigation}><div className={styles.compactVisitStepper}><span>訪問支援 {currentIndex+1} / {steps.length}</span><strong>{current}</strong><div>{currentIndex>0?<Link href={steps[currentIndex-1][1]}><ChevronLeft size={16}/>{steps[currentIndex-1][0]}</Link>:<span/>}{currentIndex<steps.length-1?<Link href={steps[currentIndex+1][1]}>{steps[currentIndex+1][0]}<ChevronRight size={16}/></Link>:null}</div></div><Subnav items={steps} current={current} /></div>;
}
...
function VisitImport() {
  ...
  const [mobileView,setMobileView]=useMobileView(sourceFieldViews,"source");
  const autoOpenedFields=useRef(false);
  const fields=(remote.value?.fields??[]).map(field=>({key:field.fieldKey,label:visitFieldLabel(field.fieldKey),valueType:field.valueType,value:String(field.textValue??field.numberValue??field.dateValue??field.booleanValue??""),confidence:field.confidence??0}));
  ...
  useEffect(()=>{if(fields.length&&mobileView!=="fields"&&!autoOpenedFields.current){autoOpenedFields.current=true;setMobileView("fields",{},"replace");}},[fields.length,mobileView]); // eslint-disable-line react-hooks/exhaustive-deps
  useEffect(()=>{
  ...
  return <><PageTitle title={isNew?"PDFから訪問を登録":"PDF取込・情報確認"} description="PDFを取り込み、抽出した情報を原本と見比べて確定します。" />{!isNew?<VisitNavigation current="訪問情報" id={id}/>:null}{remote.error?<div className={styles.recovery} role="alert"><p>訪問情報を読み込めませんでした。通信状態を確認して再読込してください。</p><button className={styles.secondaryButton} onClick={remote.refresh}><RotateCcw size={17}/>再読み込み</button></div>:null}<MobilePaneTabs value={mobileView} onChange={setMobileView} label="PDF取込の表示" items={[{value:"source",label:"PDF原本"},{value:"fields",label:"抽出結果",disabled:!hasDocument}]}/><div className={styles.split57} data-mobile-view={mobileView}>
...
function VisitPreparation({pilotContentAi}:{pilotContentAi:boolean}) {
  ...
  const [mobileView,setMobileView]=useMobileView(preparationViews,"summary");
  ...
  return <><PageTitle title="訪問前チェック" description="確定済みの訪問情報と利用可能なナレッジから生成した内容を確認します。" action={result?<button className={styles.primaryButton} disabled={checked.length!==laws.length||confirmed||action==="confirming"} onClick={()=>void complete()}>{confirmed?<><Check size={17}/>準備完了</>:action==="confirming"?"確定しています…":"確認して準備を完了"}</button>:<button className={styles.primaryButton} disabled={Boolean(processing)||action==="requesting"} onClick={()=>void request()}>{processing||action==="requesting"?"生成しています…":"訪問前チェックを生成"}</button>} /><VisitNavigation current="訪問前チェック" id={id}/>
    ...
    {result?<><MobilePaneTabs value={mobileView} onChange={setMobileView} label="訪問前チェックの工程" items={[{value:"summary",label:"1 訪問情報"},{value:"checks",label:"2 確認項目"},{value:"guidance",label:"3 トーク・Q&A"}]}/><div className={styles.prepGrid} data-mobile-view={mobileView}><section className={styles.prepSummary}><h2>今回の訪問</h2><dl><div><dt>訪問日時</dt><dd>{formatDate(workspace.value?.visit.scheduledAt)}</dd></div>{result.customerFacts.map(fact=><div key={`${fact.label}-${fact.sourceFieldKey??fact.value}`}><dt>{visitFieldLabel(fact.label)}</dt><dd>{fact.value}</dd></div>)}</dl><h3>想定される心理</h3>{result.anticipatedPsychology.map(item=><article key={item.title}><strong>{item.title}</strong><p>{item.description}</p></article>)}<Link className={styles.textButton} href={`/visits/${id}/import`}>抽出元を見直す</Link><button className={styles.mobileContinueButton} onClick={()=>setMobileView("checks")}>確認項目へ<ArrowRight size={17}/></button></section>
      <section className={styles.prepMain}><MobilePaneBack onClick={()=>setMobileView("summary")}>訪問情報へ戻る</MobilePaneBack><div className={styles.paneHeader}><div><h2>法令・接客チェック</h2><p>{checked.length} / {laws.length}項目を確認</p></div><Status>{preparation.value?.status}</Status></div><div className={styles.checkList}>{laws.map((law,index)=>{const key=`${index}-${law.title}`;return <label key={key}><input type="checkbox" checked={checked.includes(key)} disabled={confirmed} onChange={(event)=>setChecked(event.target.checked?[...checked,key]:checked.filter(item=>item!==key))}/><span><strong>{law.title}</strong><small>{law.description}</small></span></label>;})}</div>{laws.length!==4?<div className={styles.notice}><AlertTriangle size={18}/><span>法令確認が4項目揃っていません。準備を完了せず、管理者へ処理状況を連絡してください。</span></div>:null}<div className={styles.callout}><ShieldCheck size={20}/><div><strong>根拠を限定しています</strong><p>{result.contentPolicy?.usesUnapprovedContent?"確定済み抽出値と限定運用の要確認コンテンツを根拠に生成しています。原文照合が必要です。":"確定済み抽出値と承認・公開済みナレッジを根拠に生成されています。"}</p></div></div><button className={styles.mobileContinueButton} onClick={()=>setMobileView("guidance")}>想定トークとQ&amp;Aへ<ArrowRight size={17}/></button></section>
      <aside className={styles.prepAside}><MobilePaneBack onClick={()=>setMobileView("checks")}>確認項目へ戻る</MobilePaneBack><section><h2>想定トーク</h2>{result.suggestedTalks.map(item=><article key={item.title}><strong>{item.title}</strong><blockquote>{item.script}</blockquote></article>)}{!result.suggestedTalks.length?<p>生成された想定トークはありません。</p>:null}</section><section><h2>想定Q&amp;A</h2>{result.anticipatedQuestions.map((item,index)=><details open={index===0} key={item.question}><summary>{item.question}</summary><p>{item.answer}</p></details>)}{!result.anticipatedQuestions.length?<p>生成された想定Q&amp;Aはありません。</p>:null}</section><div className={styles.mobileStickyAction}><button className={styles.primaryButton} disabled={checked.length!==laws.length||confirmed||action==="confirming"} onClick={()=>void complete()}>{confirmed?<><Check size={17}/>準備完了</>:action==="confirming"?"確定しています…":"確認して準備を完了"}</button></div></aside>
    </div></>:null}
  </>;
}
...
function Transcription() {
  ...
  const [mobileView,setMobileView]=useMobileView(recordingViews,"recording");
  const recordingInput=useRef<HTMLInputElement>(null);
  const segments=useMemo(()=>remote.value?.segments??[],[remote.value?.segments]);
  const transcriptGroups=useMemo(()=>{
    const groups=new Map<string,typeof segments>();
    for(const segment of segments){const key=segment.speakerLabel?.match(/^chunk-\d+/)?.[0]??"other";groups.set(key,[...(groups.get(key)??[]),segment]);}
  ...
  const processing=latestJob&&["queued","running","retry_wait"].includes(latestJob.status);
  const transcriptionProcessing=Boolean(processing&&!transcriptId);
  const qualityProcessing=Boolean(processing&&transcriptId);
  ...
  return <><PageTitle title="録音・文字起こし" description="録音の取込状況を確認し、音声を聴きながら文字起こしを修正します。" action={<button className={styles.primaryButton} disabled={!remote.value?.transcript||!allSpeakersAssigned||confirming} onClick={()=>void confirm()}>{state==="confirmed"?<><Check size={17}/>確定しました</>:confirming?"確定しています…":"文字起こしを確定"}</button>} /><VisitNavigation current="録音・文字起こし" id={id}/>{remote.error?<div className={styles.recovery} role="alert"><p>録音と文字起こしを読み込めませんでした。通信状態を確認して再読込してください。</p><button className={styles.secondaryButton} onClick={remote.refresh}><RotateCcw size={17}/>再読み込み</button></div>:null}<details><summary>録音とAI利用の説明を確認</summary><p>{recordingConsentNotice}</p></details><MobilePaneTabs value={mobileView} onChange={setMobileView} label="録音・文字起こしの表示" items={[{value:"recording",label:"録音・処理"},{value:"transcript",label:`文字起こし${segments.length?` ${segments.length}件`:""}`,disabled:!remote.value?.recording}]}/><div className={styles.split39} data-mobile-view={mobileView}>
...
function ReviewInput() {
  const router=useRouter();
  const id=useVisitId();const remote=useRemote<VisitWorkspaceDto|null>(`workspace:${id}`,()=>resources.workspace(id));const [working,setWorking]=useState(false);const [error,setError]=useState("");const [manualText,setManualText]=useState("");const [dimensionSelection,setDimensionSelection]=useState<ReviewDimension[]|null>(null);const [mobileView,setMobileView]=useMobileView(reviewInputViews,"conversation");const transcript=remote.value?.transcript;
  ...
  return <><PageTitle title="振り返りを作成" description="録音を使う場合も使わない場合も、会話を確認して振り返りを作成できます。" /><VisitNavigation current="振り返り" id={id}/>{remote.error?<div className={styles.recovery} role="alert"><p>振り返り入力を読み込めませんでした。</p><button className={styles.secondaryButton} onClick={remote.refresh}><RotateCcw size={17}/>再読み込み</button></div>:null}{qualityBlocked?<div className={styles.qualityWarning} role="alert"><AlertTriangle/><div><strong>音声品質の確認が必要です</strong><p>混在音声や多数話者の可能性があります。録音・文字起こし画面で音声を確認し、「利用継続」を記録するか音声を差し替えてください。</p><Link className={styles.secondaryButton} href={`/visits/${id}/transcription`}>音声品質を確認</Link></div></div>:null}<MobilePaneTabs value={mobileView} onChange={setMobileView} label="振り返り作成の表示" items={[{value:"conversation",label:"会話"},{value:"criteria",label:`分析観点 ${dimensions.length}件`}]} /><div className={styles.split84} data-mobile-view={mobileView}><section className={styles.editorPane}><div className={styles.paneHeader}><div><h2>{transcript?"確定済みの会話":"録音を使わない会話入力"}</h2><p>{transcript?"修正は文字起こし画面で行います。":"録音を拒否・撤回した場合も、役割を付けた会話から振り返れます。"}</p></div>{transcript?<Link className={styles.secondaryButton} href={`/visits/${id}/transcription`}><Headphones size={17}/>録音を確認</Link>:null}</div><textarea aria-label="会話内容" className={styles.reviewTextarea} value={transcript?.fullText??manualText} onChange={event=>setManualText(event.target.value)} readOnly={Boolean(transcript)} placeholder={"査定員: 本日はどのようなお品物でしょうか。\nお客様: 時計の査定をお願いします。"}/>{!transcript?<p className={styles.muted}>1行ごとに「査定員:」または「お客様:」を付けてください。音声・録音同意は保存しません。</p>:null}{remote.loading?<p role="status">会話を読み込んでいます。</p>:null}<button className={styles.mobileContinueButton} onClick={()=>setMobileView("criteria")}>分析する観点を選ぶ<ArrowRight size={17}/></button></section><aside className={styles.conditionPane}><h2>分析する観点</h2><p className={styles.muted}>必要な観点を1項目以上選択してください。選択内容だけをAIが分析します。</p>{reviewDimensionOptions.map(option=><label key={option.id}><input type="checkbox" checked={dimensions.includes(option.id)} onChange={event=>toggleDimension(option.id,event.target.checked)}/><span>{option.label}</span></label>)}{!dimensions.length?<p role="alert">分析する観点を1項目以上選択してください。</p>:null}{sameAsLatest?<Link className={styles.primaryButton} href={`/visits/${id}/review`}>現在の結果を見る<ArrowRight size={17}/></Link>:<button className={styles.primaryButton} disabled={working||qualityBlocked||!dimensions.length||(transcript?transcript.status!=="confirmed":manualText.trim().length<10)} onClick={()=>void create()}>{working?"作成を受け付けています…":remote.value?.review?"選択した観点で再分析":"AI振り返りを作成"}<ArrowRight size={17}/></button>}{error?<p role="alert">{error}</p>:null}<p>結果は育成支援のための提案です。人事評価には使用しません。</p></aside></div></>;
}

function ReviewResult() {
  const id=useVisitId();const remote=useRemote<VisitWorkspaceDto|null>(`workspace:${id}`,()=>resources.workspace(id));const sections=(remote.value?.findings??[]).map(finding=>[finding.title,finding.description,`${finding.evidence.length}件の根拠発話`] as string[]);const evidence=(remote.value?.segments??[]).map(segment=>segment.editedText??segment.text);const [selected, setSelected] = useState(0);const [mobileView,setMobileView]=useMobileView(evidenceAnalysisViews,"analysis");
  ...
  return <><PageTitle title="AI振り返り結果" description="分析結果と、その判断につながった会話を並べて確認します。" action={<button className={styles.primaryButton} disabled={!sections.length||action==="acknowledging"||confirmed||remote.value?.review?.status==="acknowledged"} onClick={()=>void acknowledge()}><Check size={17}/>{confirmed||remote.value?.review?.status==="acknowledged"?"確認済み":action==="acknowledging"?"確認しています…":"確認を完了"}</button>} /><VisitNavigation current="振り返り" id={id}/>{remote.error?<div className={styles.recovery} role="alert"><p>振り返り結果を読み込めませんでした。通信状態を確認して再読込してください。</p><button className={styles.secondaryButton} onClick={remote.refresh}><RotateCcw size={17}/>再読み込み</button></div>:null}{reviewFailed?<div className={styles.recovery} role="alert"><p>AI振り返りを作成できませんでした。確定済み文字起こしから再実行できます。</p><button className={styles.secondaryButton} disabled={action==="retrying"} onClick={()=>void retry()}><RotateCcw size={17}/>{action==="retrying"?"再実行しています…":"振り返りを再実行"}</button></div>:null}{action==="error"?<p role="alert">操作を完了できませんでした。内容を再読込して、もう一度お試しください。</p>:null}<MobilePaneTabs value={mobileView} onChange={setMobileView} label="振り返り結果の表示" items={[{value:"analysis",label:"分析結果"},{value:"evidence",label:`根拠発話 ${evidence.length}件`}]} /><div className={styles.split57} data-mobile-view={mobileView}><section className={styles.evidencePane}><h2>会話の根拠</h2><div className={styles.audioMini}><Play size={16}/><span>確定済み文字起こし</span></div>{evidence.map((text,index)=><button data-highlight={selected===index||(selected>3&&index===2)} key={`${index}-${text}`} onClick={()=>setSelected(Math.min(index,Math.max(0,sections.length-1)))}><span>{index%2?"お客様":"査定員"}・発話 {index+1}</span><p>{text}</p></button>)}</section><section className={styles.analysisPane}><div className={styles.notice}><Sparkles size={19}/><span>{sections.length?(confirmed?"振り返りを確認済みにしました。":"AIの提案です。会話の根拠を確認して活用してください。"):reviewFailed?"振り返りの再実行が必要です。":"分析処理中です。このページは後から再確認できます。"}</span></div><div className={styles.analysisList}>{sections.map(([title,body,meta],index)=><button aria-pressed={selected===index} data-selected={selected===index} key={title} onClick={()=>{setSelected(index);setMobileView("evidence",{selected:String(index)});}}><div><span>{index+1}</span><h2>{title}</h2><small>{meta}</small></div><p>{body}</p></button>)}</div></section></div></>;
}

function ReviewHistory() {
  const params=useSearchParams();const [query,setQuery]=useState("");const remote=useRemote("history",async()=>(await resources.history()).items);const reviews=(remote.value??[]).filter(item=>`${item.caseNumber} ${item.summary}`.toLocaleLowerCase("ja").includes(query.toLocaleLowerCase("ja")));const [selectedId,setSelectedId]=useState<string|undefined>(()=>params.get("selected")??undefined);const [mobileView,setMobileView]=useMobileView(listDetailViews,"list");const selected=reviews.find(item=>item.reviewId===selectedId)??reviews[0];
  return <><PageTitle title="振り返り履歴" description="過去の訪問と学びを見返し、次の接客に活かします。" /><div className={styles.toolbar}><SearchBox value={query} onChange={setQuery} placeholder="案件番号・振り返り内容で検索"/><span>{reviews.length}件</span></div><div className={styles.listDetail} data-mobile-view={mobileView}><section className={styles.historyList}>{remote.loading?<p role="status">履歴を読み込んでいます。</p>:null}{remote.error?<p role="alert">履歴を読み込めませんでした。</p>:null}{reviews.map(item=><button data-selected={selected===item} key={item.reviewId} onClick={()=>{setSelectedId(item.reviewId);setMobileView("detail",{selected:item.reviewId});}}><span><strong>{formatDate(item.scheduledAt)}・{item.caseNumber}</strong><small>{item.summary}</small></span><Status>{item.reviewStatus==="acknowledged"?"確認済み":"完了"}</Status></button>)}</section>{selected?<aside className={styles.reviewDetail}><MobilePaneBack onClick={()=>setMobileView("list")}>振り返り一覧へ戻る</MobilePaneBack><span className={styles.kicker}>今回の学び</span><h2>{selected.caseNumber}</h2><article><strong>振り返り概要</strong><p>{selected.summary}</p></article><Link className={styles.primaryButton} href={`/visits/${selected.visitId}/review`}>振り返り結果を開く</Link></aside>:<aside className={styles.reviewDetail}><MobilePaneBack onClick={()=>setMobileView("list")}>振り返り一覧へ戻る</MobilePaneBack><h2>{remote.loading?"読み込んでいます":"履歴はありません"}</h2></aside>}</div></>;
}
...
function ContentExplorer({ title, types, reader = false }: { title: string; types: ContentType[]; reader?: boolean }) {
  ...
  const [selected, setSelected] = useState<string|undefined>(()=>params.get("selected")??undefined);
  const [mobileView,setMobileView]=useMobileView(knowledgeViews,"filters");
  ...
  return <><PageTitle title={title} description="公開済みの現場知識から、今の状況に合う内容を探します。" /><Subnav items={knowledgeTabs} current={title} /><MobilePaneTabs value={mobileView} onChange={setMobileView} label="知識検索の表示" items={[{value:"filters",label:"検索"},{value:"list",label:"結果"},{value:"detail",label:"本文",disabled:!detail}]}/><div className={reader ? styles.knowledgeReaderGrid : styles.knowledgeGrid} data-mobile-view={mobileView}>
    <aside className={styles.categoryPane}><SearchBox value={query} onChange={value=>{setQuery(value);setPage(1);}} /><h2>カテゴリ</h2><button data-selected={category === "すべて"} onClick={() => setCategory("すべて")}>すべて<span>{total}</span></button>{categories.map((item) => <button data-selected={category === item} key={item} onClick={() => setCategory(item)}>{item}</button>)}<button className={styles.mobileSearchResults} onClick={()=>setMobileView("list")}>検索結果を見る<span>{visible.length}件</span></button></aside>
    <section className={styles.resultPane} aria-label="検索結果一覧"><MobilePaneBack onClick={()=>setMobileView("filters")}>検索条件へ戻る</MobilePaneBack><div className={styles.paneHeader}><div><h2>検索結果</h2><p><span>{total.toLocaleString()}件</span>・<small>{page}ページ</small></p></div></div><div className={styles.resultList}>{visible.map((item) => <button data-selected={selectedId === item.id} key={item.id} onClick={() => {setSelected(item.id);setMobileView("detail",{selected:item.id});}}><span>{item.category}</span><strong>{item.title}</strong><small>{typeLabel(item.type)}</small></button>)}</div><div className={styles.toolbar}><button className={styles.secondaryButton} disabled={page===1} onClick={()=>{setPage(current=>Math.max(1,current-1));setSelected(undefined);}}>前のページ</button><button className={styles.secondaryButton} disabled={!hasMore} onClick={()=>{setPage(current=>current+1);setSelected(undefined);}}>次のページ</button></div></section>
    <article className={styles.contentDetail} tabIndex={0} aria-label="コンテンツ詳細"><MobilePaneBack onClick={()=>setMobileView("list")}>検索結果へ戻る</MobilePaneBack>{detail ? <><div className={styles.detailHeader}><span>{typeLabel(detail.type)}・{detail.category}</span><h2>{detail.title}</h2><div>{detail.tags.map((tag) => <em key={tag}>{tag}</em>)}</div></div><div className={styles.readableBody}>{renderContentBody(detail)}</div>{detail.type === "talk" ? <TalkGuidance detail={detail} /> : null}</> : <div className={styles.emptyDetail}><BookOpen /><p>一覧から内容を選択してください。</p></div>}</article>
  </div></>;
}
...
function ContentExplorerInner({ types }: { types: ContentType[] }) {
  ...
  const [selected, setSelected] = useState<string|undefined>(()=>params.get("selected")??undefined);
  const [mobileView,setMobileView]=useMobileView(listDetailViews,"list");
  const selectedId = items.some((item) => item.id === selected) ? selected : items[0]?.id;
  const detail = useContentDetail(selectedId);
  const subjectLabel = types[0] === "price" ? "券種" : "用語";
  return <><MobilePaneTabs value={mobileView} onChange={setMobileView} label="用語・価格の表示" items={[{value:"list",label:"一覧"},{value:"detail",label:"詳細",disabled:!detail}]}/><div className={styles.referenceGrid} data-mobile-view={mobileView}><section className={styles.referenceTable}><div className={styles.toolbar}><SearchBox value={query} onChange={value=>{setQuery(value);setPage(1);}} /><span>{total}件・{page}ページ</span></div><table><thead><tr><th>{subjectLabel}</th><th>カテゴリ</th><th>詳細</th></tr></thead><tbody>{items.map((item) => <tr data-selected={selectedId === item.id} key={item.id}><td data-label={subjectLabel}><button className={styles.tableSelectButton} aria-pressed={selectedId === item.id} onClick={() => {setSelected(item.id);setMobileView("detail",{selected:item.id});}}><strong>{item.title}</strong></button></td><td data-label="カテゴリ">{item.category}</td><td data-label="詳細"><ChevronRight size={16} /></td></tr>)}</tbody></table><div className={styles.toolbar}><button className={styles.secondaryButton} disabled={page===1} onClick={()=>{setPage(current=>Math.max(1,current-1));setSelected(undefined);}}>前のページ</button><button className={styles.secondaryButton} disabled={!hasMore} onClick={()=>{setPage(current=>current+1);setSelected(undefined);}}>次のページ</button></div></section><article className={styles.contentDetail} tabIndex={0} aria-label="コンテンツ詳細"><MobilePaneBack onClick={()=>setMobileView("list")}>一覧へ戻る</MobilePaneBack>{detail ? <><div className={styles.detailHeader}><span>{detail.category}</span><h2>{detail.title}</h2></div><div className={styles.readableBody}>{renderContentBody(detail)}</div></> : null}</article></div></>;
}
...
function Videos() {
  ...
  return <><PageTitle title="動画ライブラリ" description="公開済みの研修動画と文字版を確認できます。" /><Subnav items={trainingTabs} current="動画ライブラリ" /><MobilePaneTabs value={mobileView} onChange={setMobileView} label="動画ライブラリの表示" items={[{value:"list",label:"動画一覧"},{value:"detail",label:"再生・文字版",disabled:!detail}]}/><div className={styles.videoGrid} data-mobile-view={mobileView}><aside className={styles.videoList}><h2>研修動画</h2><p>{total}件</p>{items.map(item=><button data-selected={selectedId===item.id} onClick={()=>{setSelected(item.id);setMobileView("detail",{selected:item.id});}} key={item.id}><span className={styles.thumbnail}><Play /></span><span><strong>{item.title}</strong><small>{item.category}</small></span></button>)}{!items.length?<p>公開済みの研修動画はありません。</p>:null}</aside><section className={styles.videoPlayer}><MobilePaneBack onClick={()=>setMobileView("list")}>動画一覧へ戻る</MobilePaneBack>{detail?<>{videoUrl?<video className={styles.player} controls preload="metadata" src={videoUrl}>動画を再生できません。</video>:<div className={styles.player}><BookOpen/><span>動画ファイルはまだ登録されていません</span></div>}<h2>{detail.title}</h2><p>{detail.body||"文字版は登録されていません。"}</p></>:<><h2>動画を選択してください</h2><p>公開後の動画がここに表示されます。</p></>}</section></div></>;
}
...
function ContentsAdmin() {
  ...
  const [mobileView,setMobileView]=useMobileView(adminTripleViews,"list");
  ...
  return <><PageTitle title="コンテンツ管理" description="現場で使う内容を一覧、編集、プレビューで確認します。" action={<button className={styles.primaryButton} onClick={()=>{setCreating(true);setSelected(undefined);setSaved(false);setError("");setMobileView("edit");}}>新しい下書き</button>} /><Subnav items={adminTabs} current="コンテンツ"/><p className={styles.mobilePcRecommendation}>スマートフォンでも主要操作ができます。長文の版比較・高度編集はPC表示を推奨します。</p><MobilePaneTabs value={mobileView} onChange={setMobileView} label="コンテンツ管理の工程" items={[{value:"list",label:"1 一覧"},{value:"edit",label:"2 編集",disabled:!creating&&!detail},{value:"preview",label:"3 確認",disabled:!detail}]}/><div className={styles.adminTriple} data-mobile-view={mobileView}><section className={styles.adminList}><div className={styles.toolbar}><select aria-label="コンテンツ種別" value={type} onChange={event=>{setType(event.target.value as ContentType);setSelected(undefined);setCreating(false);setSaved(false);setVideoFile(null);setVideoState("idle");}}>{["talk","flow","glossary","price","manual","legal","video","roleplay"].map(value=><option value={value} key={value}>{typeLabel(value as ContentType)}</option>)}</select><span>{total}件</span></div><div className={styles.resultList}>{items.map(item=><button data-selected={!creating&&selectedId===item.id} onClick={()=>{setSelected(item.id);setCreating(false);setSaved(false);setError("");setMobileView("edit",{selected:item.id});}} key={item.id}><span>{item.category}</span><strong>{item.title}</strong><small>{publicationStateLabel(item.publicationState).label}</small></button>)}</div></section><form className={styles.adminEditor} key={editorKey} onSubmit={event=>void save(event)}><MobilePaneBack onClick={()=>setMobileView("list")}>一覧へ戻る</MobilePaneBack><h2>{creating?"新しい下書き":"編集"}</h2><label>タイトル<input name="title" maxLength={300} required defaultValue={creating?"":detail?.title??""}/></label><label>カテゴリ<input name="category" maxLength={200} defaultValue={creating?"":detail?.category??""}/></label><label>本文<textarea name="body" required defaultValue={creating?"":detail?.body??""} rows={14}/></label>{type==="video"?<label>動画ファイル（MP4 / WebM・2GB・4時間以内）<input type="file" accept="video/mp4,video/webm" onChange={event=>{setVideoFile(event.target.files?.[0]??null);setVideoState("idle");}}/>{videoState==="working"?<small role="status">動画を検査・保存しています…</small>:videoState==="done"?<small>動画を保存しました</small>:null}</label>:null}{error?<p role="alert">{error}</p>:null}<button className={styles.primaryButton} type="submit" disabled={(!creating&&!detail)||videoState==="working"}>{saved?<><Check size={17}/>保存しました</>:"下書きを保存"}</button><button className={styles.secondaryButton} type="button" disabled={creating||!detail||!publishVersion} onClick={()=>void publish()}>現在の版を公開</button>{detail?<button className={styles.mobileContinueButton} type="button" onClick={()=>setMobileView("preview")}>プレビューへ<ArrowRight size={17}/></button>:null}</form><article className={styles.adminPreview}><MobilePaneBack onClick={()=>setMobileView("edit")}>編集へ戻る</MobilePaneBack><span className={styles.kicker}>プレビュー</span>{type==="video"&&previewVideoUrl?<video className={styles.player} controls preload="metadata" src={previewVideoUrl} aria-label="下書き動画プレビュー">動画を再生できません。</video>:null}<h2>{creating?"新しいコンテンツ":detail?.title}</h2><p>{creating?"保存後に内容を確認できます。":detail?.body}</p></article></div></>;
}

function UsersAdmin({viewerId}:{viewerId?:string}) {
  const params=useSearchParams();const remote=useRemote("users",async()=>(await resources.users()).items);const users=remote.value??[];const [selectedId,setSelectedId]=useState<string|undefined>(()=>params.get("selected")??undefined);const [mobileView,setMobileView]=useMobileView(listDetailViews,"list");const [confirming,setConfirming]=useState<"role"|"suspend"|null>(null);const [pendingRoles,setPendingRoles]=useState<string[]|null>(null);const [inviting,setInviting]=useState(false);const [inviteState,setInviteState]=useState<"idle"|"working"|"error">("idle");const selected=users.find(user=>user.id===selectedId)??users[0];const roleName=(role:string)=>({assessor:"査定員",manager:"管理者",educator:"教育担当",content_approver:"承認担当",system_admin:"システム管理者"}[role]??role);const roleOptions=["assessor","manager","educator","content_approver"];
...
function OperationsHealth({onOpenJobs}:{onOpenJobs:()=>void}){
  ...
  const alertLabels:Record<OperationsHealthDto["alerts"][number]["failureClass"],string>={STT_HEARTBEAT_STALE:"文字起こしの更新停止",STT_LRO_TIMEOUT:"Chirp 3処理の長時間化",RETRY_WAIT_OVERDUE:"再試行の遅延",MODEL_OUTPUT_INVALID:"AI出力形式の不整合",EVIDENCE_INVALID:"振り返り根拠の不整合",RETRY_LIMIT_EXCEEDED:"再試行上限到達",MARKET_PRICE_STALLED:"落札相場の取得停止",MARKET_PRICE_BLOCKED:"落札相場の安全停止"};
