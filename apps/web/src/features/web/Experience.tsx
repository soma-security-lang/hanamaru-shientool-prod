...
import {resources,type ContentPolicyDto,type JobDto,type OperationsHealthDto,type PreparationDto,type RetentionBindingDto,type RetentionPolicyDto,type TranscriptQualityAssessmentDto,type TranscriptQualityFlag,type VisitWorkspaceDto} from "@/lib/api/resources";
...
function Transcription() {
  ...
  const speakerLabels=Array.from(new Set(segments.map(segment=>segment.speakerLabel).filter((label):label is string=>Boolean(label))));
  ...
  function assignSpeakerLabel(label:string,role:"staff"|"customer"|"unknown"){
    setSpeakerRoles(current=>Object.fromEntries([...Object.entries(current),...segments.filter(segment=>segment.speakerLabel===label).map(segment=>[segment.id,role])]) as Record<string,"staff"|"customer"|"unknown">);
  }
...
function ReviewInput() {
  const router=useRouter();
  const id=useVisitId();const remote=useRemote<VisitWorkspaceDto|null>(`workspace:${id}`,()=>resources.workspace(id));const [working,setWorking]=useState(false);const [error,setError]=useState("");const [manualText,setManualText]=useState("");const transcript=remote.value?.transcript;
...
function ReviewResult() {
  ...
  useEffect(()=>{if(!processing)return;const timer=window.setInterval(remote.refresh,1500);return()=>window.clearInterval(timer);},[processing]); // eslint-disable-line react-hooks/exhaustive-deps
  async function retry(){const transcript=remote.value?.transcript;if(!transcript)return;setAction("retrying");try{await resources.requestReview(transcript.id,"接客品質と次回の行動を6領域で確認");remote.refresh();setAction("idle");}catch{setAction("error");}}
