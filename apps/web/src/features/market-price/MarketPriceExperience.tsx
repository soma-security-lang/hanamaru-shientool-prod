...
function sourceLabel(value:MarketPriceSearchDto["sourceProvider"]){return value==="aucfan_api"?"オークファン":"ヤフオク";}
function candidateEndLabel(candidate:MarketPriceCandidateDto){return candidate.endedOn??(candidate.endedAt?new Date(candidate.endedAt).toLocaleDateString("ja-JP"):"日付不明");}
...
function IdentificationView({searchId,options,go,setGlobalError}:{searchId:string;options?:MarketPriceOptionsDto;go:Navigate;setGlobalError:(value:string)=>void}){
  const [item,setItem]=useState<MarketPriceIdentificationDto>();const [fields,setFields]=useState<MarketPriceIdentificationFields>();const [busy,setBusy]=useState(false);const [sourceMode,setSourceMode]=useState<MarketPriceSearchMode>("yahoo");
  ...
  return <div className={styles.identifyLayout}>
    ...
    <section className={styles.panel}><header><span>検索条件</span><h2>検索キーワードを1件選択</h2><p>同じ確認済み条件で、取得元を切り替えて検証できます。</p></header><div className={styles.queryList}>{allQueries.map(query=><label key={query.id} data-selected={query.decision==="accepted"}><input type="radio" name="query" checked={query.decision==="accepted"} onChange={()=>setQuery(query)}/><span><strong>{query.keyword}</strong><small>{query.source==="ai"?"AI提案":query.source==="edited"?"編集済み":"利用者入力"}・{query.breadth==="strict"?"厳密":query.breadth==="broad"?"広め":"標準"}</small></span></label>)}</div>{fields.searchQueries.filter(query=>query.decision==="accepted").map(query=><label className={styles.editQuery} key={query.id}>選択中の検索キーワード<input value={query.keyword} onChange={event=>setFields({...fields,searchQueries:fields.searchQueries.map(item=>item.id===query.id?{...item,keyword:event.target.value,source:"edited"}:item)})}/></label>)}<fieldset className={styles.sourcePicker}><legend>取得元</legend>{options?.sources.map(source=><label key={source.value} data-selected={sourceMode===source.value} data-disabled={!source.enabled}><input type="radio" name="sourceMode" checked={sourceMode===source.value} disabled={!source.enabled} onChange={()=>setSourceMode(source.value)}/><span><strong>{source.label}</strong><small>{source.description}</small></span></label>)}</fieldset>{options?.sources.find(source=>source.value===sourceMode)?.limitations.map(limitation=><p className={styles.sourceLimitation} key={limitation}><Info size={17}/>{limitation}</p>)}</section>
    <footer className={styles.stickyAction}><button type="button" className={styles.secondaryButton} onClick={()=>go("input")}><ArrowLeft size={18}/>入力へ戻る</button><button type="button" className={styles.primaryButton} disabled={busy} onClick={()=>void confirm()}>{busy?<LoaderCircle className={styles.spin} size={18}/>:<Search size={18}/>}選択した取得元で検索</button></footer>
  </div>;
}

function ProgressView({searchId,compareSearchId,go,setGlobalError}:{searchId:string;compareSearchId:string;options?:MarketPriceOptionsDto;go:Navigate;setGlobalError:(value:string)=>void}){
  const [item,setItem]=useState<MarketPriceSearchDto>();const [compared,setCompared]=useState<MarketPriceSearchDto>();const [busy,setBusy]=useState(false);
  const load=useCallback(async()=>{if(!searchId)return true;const [primary,comparison]=await Promise.allSettled([resources.marketPriceSearch(searchId),compareSearchId?resources.marketPriceSearch(compareSearchId):Promise.resolve(undefined)]);if(primary.status==="rejected"&&comparison.status==="rejected"){setGlobalError(errorMessage(primary.reason));return true;}const value=primary.status==="fulfilled"?primary.value:comparison.status==="fulfilled"?comparison.value:undefined;const paired=primary.status==="fulfilled"&&comparison.status==="fulfilled"?comparison.value:undefined;if(!value){setGlobalError("検索状態を取得できませんでした。");return true;}setItem(value);setCompared(paired);if(primary.status==="rejected"||comparison.status==="rejected")setGlobalError("片方の取得元を確認できないため、取得できた結果だけを表示しています。");if(!paired&&["ready","partial","confirmed"].includes(value.status)){go(value.status==="confirmed"?"result":"candidates",value.id,"replace");return true;}return terminalStatuses.has(value.status)&&(!paired||terminalStatuses.has(paired.status));},[searchId,compareSearchId,go,setGlobalError]);
  useEffect(()=>{let active=true;let timer=0;async function poll(){const done=await load();if(active&&!done)timer=window.setTimeout(poll,document.visibilityState==="visible"?2000:10000);}void poll();return()=>{active=false;window.clearTimeout(timer);};},[load]);
  ...
  return <section className={styles.progressPanel}><div className={styles.statusIcon} data-status={item.status}>{item.status==="blocked"||item.status==="failed"?<ShieldAlert size={30}/>:item.status==="cancelled"?<X size={30}/>:<LoaderCircle className={!terminalStatuses.has(item.status)?styles.spin:undefined} size={30}/>}</div><span>{compared?"取得元を比較中":statusLabel(item.status)}</span><h2>{item.status==="blocked"?"安全のため取得を停止しました":item.status==="failed"?"取得を完了できませんでした":item.status==="cancelled"?"検索を取り消しました":"落札相場を確認しています"}</h2><p>取得件数ではなく実際の処理状態を表示しています。画面を閉じても処理は続きます。</p>{compared?<div className={styles.sourceComparison}>{[item,compared].map((source,index)=>{const other=index===0?compared.id:item.id;const available=["ready","partial","confirmed"].includes(source.status);return <article key={source.id}><span>{sourceLabel(source.sourceProvider)}</span><strong>{statusLabel(source.status)}</strong><small>{source.candidateCount}件取得・中央値 {source.medianPrice==null?"—":yen.format(source.medianPrice)}</small>{available?<button type="button" className={styles.secondaryButton} onClick={()=>go(source.status==="confirmed"?"result":"candidates",source.id,"replace",other)}>結果を確認</button>:source.failureClass?<small>停止理由: {source.failureClass}</small>:null}</article>;})}</div>:<ol>{stages.map((stage,index)=><li key={stage} data-active={index<=(["queued","planning"].includes(item.status)?0:item.status==="fetching"?1:item.status==="normalizing"?2:3)}><Check size={17}/><span>{stage}</span></li>)}</ol>}{item.failureClass?<p className={styles.failure}>停止理由: {item.failureClass}</p>:null}<div className={styles.actionRow}>{!compared&&["blocked","failed"].includes(item.status)?<button type="button" className={styles.primaryButton} disabled={busy} onClick={()=>void act("retry")}><RefreshCw size={18}/>途中から再試行</button>:null}{!compared&&!["blocked","failed","cancelled"].includes(item.status)?<button type="button" className={styles.secondaryButton} disabled={busy} onClick={()=>void act("cancel")}>検索を取り消す</button>:null}<button type="button" className={styles.secondaryButton} onClick={()=>go("history")}>履歴を見る</button></div></section>;
}

function SourceSwitcher({item,compareSearchId,view,go}:{item:MarketPriceSearchDto;compareSearchId:string;view:"candidates"|"result";go:Navigate}){
  return <div className={styles.sourceSwitcher} aria-label="相場の取得元"><span>表示中: <strong>{sourceLabel(item.sourceProvider)}</strong></span>{compareSearchId?<button type="button" onClick={()=>go(view,compareSearchId,"replace",item.id)}>{item.sourceProvider==="yahoo_scrape"?"オークファン":"ヤフオク"}の結果へ切替</button>:null}</div>;
}

function CandidatesView({searchId,compareSearchId,options,go,setGlobalError}:{searchId:string;compareSearchId:string;options?:MarketPriceOptionsDto;go:Navigate;setGlobalError:(value:string)=>void}){
  ...
  return <div className={styles.candidateWorkspace}>
    <SourceSwitcher item={item} compareSearchId={compareSearchId} view="candidates" go={go}/>
    ...
    <section className={styles.candidatePanel}>
      ...
      {item.coverageStatus!=="complete"?<div className={styles.partial}><AlertTriangle size={22}/><span><strong>一部の取得結果です</strong><small>直近90日の取得完了は確認できていません。取得できた範囲の参考値として確認してください。</small></span></div>:null}{(item.sourceLimitations??[]).map(limitation=><p className={styles.sourceLimitation} key={limitation}><Info size={17}/>{limitation}</p>)}
      {candidates.length?<>
        <div className={styles.candidateTableWrap}>
          <table className={styles.candidateTable}>
            ...
            <tbody>{candidates.map(candidate=><tr key={candidate.id} data-included={candidate.included}>
              <td><span className={styles.stateBadge}>{conditionLabel(candidate.normalizedCondition,options)}</span><strong>{candidate.title}</strong><small>{candidate.matchReasons.join("・")||"検索条件に一致"}・{candidateEndLabel(candidate)}終了</small></td>
...
function ResultView({searchId,compareSearchId,go,setGlobalError}:{searchId:string;compareSearchId:string;options?:MarketPriceOptionsDto;go:Navigate;setGlobalError:(value:string)=>void}){
