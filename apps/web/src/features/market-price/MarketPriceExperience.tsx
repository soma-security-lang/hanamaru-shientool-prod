...
function CandidatesView({searchId,options,go,setGlobalError}:{searchId:string;options?:MarketPriceOptionsDto;go:(view:View,id?:string,mode?:"push"|"replace")=>void;setGlobalError:(value:string)=>void}){
  ...
  async function toggle(candidateId:string,included:boolean){const decision=included?"exclude":"include";setItem(current=>current?withCandidateStatistics(current,current.candidates.map(candidate=>candidate.id===candidateId?{...candidate,inclusionOverride:decision,included:!included,decisionSource:"manual"}:candidate)):current);setBusy(candidateId);try{await resources.overrideMarketPriceCandidate(search.id,candidateId,decision);await load();}catch(reason){setGlobalError(errorMessage(reason));await load();}finally{setBusy("");}}
  async function applyPolicy(){setItem(current=>current?previewOutlierPolicy(current,policy):current);setBusy("policy");try{await resources.updateMarketPriceOutlierPolicy(search.id,policy);await load();}catch(reason){setGlobalError(errorMessage(reason));await load();}finally{setBusy("");}}
  async function confirm(){setBusy("confirm");try{const latest=await resources.marketPriceSearch(search.id);await resources.confirmMarketPriceSearch(search.id,latest.lockVersion);go("result",search.id,"replace");}catch(reason){setGlobalError(errorMessage(reason));await load();}finally{setBusy("");}}
  return <div className={styles.candidateWorkspace}>
    <aside className={styles.filterPanel}><header><SlidersHorizontal size={20}/><div><span>検索条件</span><h2>候補を絞り込む</h2></div></header><dl><div><dt>期間</dt><dd>{new Date(item.periodStart).toLocaleDateString("ja-JP")}〜{new Date(item.periodEnd).toLocaleDateString("ja-JP")}</dd></div><div><dt>商品状態</dt><dd>{item.conditions.map(value=>conditionLabel(value,options)).join("、")}</dd></div><div><dt>取得完全性</dt><dd>{item.coverageStatus==="complete"?"直近90日の取得完了":"一部の取得結果"}</dd></div></dl><fieldset><legend>候補の表示</legend>{(["all","included","excluded"] as const).map(value=><label key={value}><input type="radio" checked={filter===value} onChange={()=>setFilter(value)}/>{value==="all"?"すべて":value==="included"?"集計対象":"除外"}</label>)}</fieldset><div className={styles.policy}><label><input type="checkbox" checked={policy.enabled} onChange={event=>setPolicy({...policy,enabled:event.target.checked})}/>価格外れ値を自動除外</label><label>中央値からの乖離率<strong>{Math.round(policy.deviationThreshold*100)}%</strong><input type="range" min="5" max="100" step="5" value={Math.round(policy.deviationThreshold*100)} onChange={event=>setPolicy({...policy,deviationThreshold:Number(event.target.value)/100})}/></label><small>同一状態が{policy.minimumGroupSize}件以上ある場合に判定します。</small><button type="button" onClick={()=>void applyPolicy()} disabled={busy==="policy"}>{busy==="policy"?"再計算中…":"基準を再適用"}</button></div></aside>
        ...
        <div className={styles.candidateTableWrap}>
          <table className={styles.candidateTable}>
            ...
            <tbody>{candidates.map(candidate=><tr key={candidate.id} data-included={candidate.included}>
              ...
              <td><button type="button" disabled={busy===candidate.id} onClick={()=>void toggle(candidate.id,candidate.included)}>{candidate.included?"集計から除外":"集計へ戻す"}</button></td>
            </tr>)}</tbody>
          </table>
        </div>
        <div className={styles.candidateCards}>{candidates.map(candidate=><article key={candidate.id} data-included={candidate.included}><div className={styles.candidateMain}><span className={styles.stateBadge}>{conditionLabel(candidate.normalizedCondition,options)}</span><h3>{candidate.title}</h3><p>{candidate.matchReasons.join("・")||"検索条件に一致"}</p><small>{new Date(candidate.endedAt).toLocaleDateString("ja-JP")} 終了</small></div><div className={styles.candidatePrice}><strong>{yen.format(candidate.closingPrice)}</strong>{candidate.priceDeviationRate!=null?<small>中央値差 {Math.round(candidate.priceDeviationRate*100)}%</small>:null}</div><div className={styles.candidateDecision}>{candidate.exclusionReasons.length?<small>{candidate.exclusionReasons.map(reasonLabel).join("・")}</small>:<small>自動採用</small>}<button type="button" disabled={busy===candidate.id} onClick={()=>void toggle(candidate.id,candidate.included)}>{candidate.included?"集計から除外":"集計へ戻す"}</button></div></article>)}</div>
      </>:<p className={styles.emptyList}>条件に一致する候補はありません。</p>}
    </section>
    <aside className={styles.summaryPanel}><span>現在の集計</span><h2>{item.medianPrice==null?"—":yen.format(item.medianPrice)}</h2><p>中央値</p><dl><div><dt>最低価格</dt><dd>{item.minimumPrice==null?"—":yen.format(item.minimumPrice)}</dd></div><div><dt>最高価格</dt><dd>{item.maximumPrice==null?"—":yen.format(item.maximumPrice)}</dd></div><div><dt>採用件数</dt><dd>{item.includedCount}件</dd></div><div><dt>除外件数</dt><dd>{Math.max(0,item.candidateCount-item.includedCount)}件</dd></div></dl><button type="button" className={styles.primaryButton} disabled={busy==="confirm"||item.includedCount<1} onClick={()=>void confirm()}><CheckCircle2 size={18}/>{busy==="confirm"?"確定中…":"この相場を確定"}</button></aside>
  </div>;
}
