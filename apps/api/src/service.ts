...
import type { Capability,MarketPriceOutlierPolicy,MarketPriceSourceProvider,ProductCondition, RequestContext } from "@hanamaru/contracts";
...
function marketPriceSearchDto(row:unknown,{includeEmptyCandidates=false}:{includeEmptyCandidates?:boolean}={}):Json{
  ...
  value.sourceLimitations=value.sourceLimitationsJson??[];
  ...
  delete value.sourceLimitationsJson;
  return value;
}
...
export class BackendService {
  ...
  async createMarketPriceSearch(ctx:RequestContext,key:string|undefined,b:Json){
    ...
    const searchBasis=String(b.searchBasis??"keyword");
    if(!["keyword","model_number"].includes(searchBasis))throw invalid("検索方法を確認してください");
    const selectedSearchQueryId=b.selectedSearchQueryId==null?null:text(b.selectedSearchQueryId,100,"selectedSearchQueryId");
    if(searchBasis==="keyword"&&!selectedSearchQueryId)throw invalid("検索キーワードを1件選択してください");
    if(searchBasis==="model_number"&&!identificationId)throw invalid("型番検索では商品特定結果を指定してください");
    ...
    const sourceProvider=(b.sourceProvider??"yahoo_scrape") as MarketPriceSourceProvider;
    if(!["yahoo_scrape","aucfan_api"].includes(sourceProvider))throw invalid("取得元を確認してください");
    ...
    return this.write(ctx,"market_price.search.create",key,b,"market_price.search.requested","market_price_search",async tx=>{
      const flags=await this.flags(tx,ctx.organizationId);if(!flags.market_price_search)throw new ApiProblem("FEATURE_DISABLED",404,"買取相場（仮）は現在利用できません");
      if(sourceProvider==="aucfan_api"&&!flags.market_price_aucfan)throw new ApiProblem("FEATURE_DISABLED",404,"オークファン比較は現在利用できません");
      if(!access.organization&&!access.branchIds.includes(ctx.branchId)&&!access.self)throw denied();
      ...
      const queries=Array.isArray(confirmed.searchQueries)?confirmed.searchQueries:[];const selected=selectedSearchQueryId?queries.find(raw=>raw&&typeof raw==="object"&&String((raw as Json).id)===selectedSearchQueryId) as Json|undefined:undefined;
      const modelNumber=normalizeSearchKeyword(typeof confirmed.modelNumber==="string"?confirmed.modelNumber:"");
      if(searchBasis==="keyword"&&!selected)throw invalid("選択した検索語を確認してください");
      if(searchBasis==="model_number"&&!modelNumber)throw invalid("型番を入力してから型番優先検索を開始してください");
      const keyword=searchBasis==="model_number"?modelNumber:normalizeSearchKeyword(String(selected?.keyword??""));if(!keyword)throw invalid("検索条件を確認してください");
      ...
      const queryJson={searchBasis,selectedSearchQueryId,keyword,modelNumber:searchBasis==="model_number"?modelNumber:null,category:typeof confirmed.category==="string"?confirmed.category:null,brand:typeof confirmed.brand==="string"?confirmed.brand:null,excludeKeywords};
      ...
      const normalizedQueryHash=sha(JSON.stringify({sourceProvider,queryJson,conditions,outlierPolicy,periodDays:90,sort:"ENDED_AT_NEWEST",pageSize:100,parserVersion:sourceProvider==="yahoo_scrape"?YAHOO_RESULT_PARSER_VERSION:"aucfan-api-1.0.0",generatorVersion:sourceProvider==="yahoo_scrape"?YAHOO_URL_GENERATOR_VERSION:"aucfan-query-1.0.0",registryFingerprint:sourceProvider==="yahoo_scrape"?registryFingerprint:null}));
      ...
      await tx.query("INSERT INTO market_price_searches(id,organization_id,branch_id,identification_id,created_by_membership_id,status,query_json,condition_filters_json,outlier_policy_json,normalized_query_hash,period_start,period_end,started_at,source_provider) VALUES($1,$2,$3,$4,$5,'queued',$6,$7::jsonb,$8,$9,$10,$11,$11,$12)",[searchId,ctx.organizationId,ctx.branchId,resolvedIdentificationId,ctx.membershipId,queryJson,JSON.stringify(conditions),outlierPolicy,normalizedQueryHash,period.periodStart,period.periodEnd,sourceProvider]);
  ...
  async getMarketPriceSearch(ctx:RequestContext,id:string){
    const access=this.marketPriceAccess(ctx,"market_price:read");return this.read(ctx,"market_price.search.read","market_price_search",async tx=>{
      const found=await tx.query("SELECT id,identification_id,job_id,source_provider,status,coverage_status,period_start,period_end,period_days,sort_key,page_size,max_pages,query_json,condition_filters_json,outlier_policy_json,ai_parameter_plan_json,parameter_registry_version,generator_version,planner_model,planner_prompt_version,planner_status,candidate_count,included_count,minimum_price,median_price_before_outlier_exclusion,median_price,maximum_price,exclusion_counts_json,source_limitations_json,parser_version,failure_class,result_id,confirmed_at,lock_version,started_at,completed_at,created_at,updated_at FROM market_price_searches WHERE organization_id=$1 AND id=$2 AND ($3::boolean OR branch_id=ANY($4::uuid[]) OR ($5::boolean AND created_by_membership_id=$6))",[ctx.organizationId,id,access.organization,access.branchIds,access.self,ctx.membershipId]);
      if(!found.rows[0])throw notFound();const candidates=await tx.query("SELECT id,source_item_id,source_type,source_provider,canonical_url,title,closing_price,ended_at,ended_on::text,ended_at_precision,normalized_condition,match_score,match_reasons,condition_matched,tax_display,exclusion_reasons,condition_group_count,condition_median_price,price_deviation_rate,iqr_lower_bound,iqr_upper_bound,auto_outlier,inclusion_override,included,decision_source FROM market_price_candidates WHERE organization_id=$1 AND search_id=$2 ORDER BY COALESCE(ended_at,ended_on::timestamptz) DESC,source_item_id LIMIT 2000",[ctx.organizationId,id]);return{...marketPriceSearchDto(found.rows[0]),candidates:candidates.rows.map(marketPriceCandidateDto)};
    });
  }
  ...
  async updateMarketPriceOutlierPolicy(ctx:RequestContext,searchId:string,key:string|undefined,b:Json){
    ...
    return this.write(ctx,"market_price.outlier_policy.update",key,b,"market_price.outlier_policy.updated","market_price_search",async tx=>{
      ...
      const rows=await tx.query<any>("SELECT id,source_item_id,source_type,source_provider,canonical_url,title,closing_price::text,ended_at,ended_on::text,condition_label,normalized_condition,tax_display,content_hash,inclusion_override FROM market_price_candidates WHERE organization_id=$1 AND search_id=$2 ORDER BY COALESCE(ended_at,ended_on::timestamptz) DESC,source_item_id",[ctx.organizationId,searchId]);
      const query=search.query_json as Json;const evaluated=evaluateMarketPriceCandidates({items:rows.rows.map(row=>({sourceProvider:row.source_provider,sourceItemId:row.source_item_id,sourceType:row.source_type,canonicalUrl:row.canonical_url??"",title:row.title,closingPrice:Number(row.closing_price),endedAt:row.ended_at?new Date(row.ended_at).toISOString():new Date(`${row.ended_on}T12:00:00+09:00`).toISOString(),endedOn:row.ended_on??undefined,endedAtPrecision:row.ended_at?"timestamp":"date",sourceCondition:row.condition_label,normalizedCondition:row.normalized_condition,taxDisplay:row.tax_display,categoryId:null,brandId:null,contentHash:row.content_hash})),selectedKeyword:String(query.keyword??""),searchBasis:query.searchBasis==="model_number"?"model_number":"keyword",modelNumber:typeof query.modelNumber==="string"?query.modelNumber:null,periodStart:new Date(search.period_start),periodEnd:new Date(search.period_end),selectedConditions:search.condition_filters_json,excludeKeywords:Array.isArray(query.excludeKeywords)?query.excludeKeywords.map(String):[],outlierPolicy:policy});
