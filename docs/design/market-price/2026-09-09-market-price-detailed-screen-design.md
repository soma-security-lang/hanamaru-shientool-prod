...
利用者は検索開始前に、次のどちらかを明示的に選択する。

| Value | Label | 外部検索語 | 同一商品判定 |
| --- | --- | --- | --- |
| `keyword` | キーワードで検索 | 採用済み検索キーワード1件 | 検索語tokenの一致度 |
| `model_number` | 型番を優先して検索 | 確認済み型番 | NFKC正規化後の型番一致を必須とする |

- 初期値は`keyword`とし、既存検索との互換性を維持する。
- 型番が空の場合、`model_number`は選択不可とする。
- 型番優先では全角・半角、空白、ハイフン等の区切り差を正規化する。
- 型番の末尾に英数字が連続する派生型番は別商品として除外する。例：`CFI-2000A01`に対する`CFI-2000A01B`。
- Yahoo／オークファンの取得元選択は検索方法から独立させる。
- 候補確認、結果、履歴では検索方法と使用型番を表示する。
...
| 型番を優先して検索 | 確認済み型番あり、商品状態1件以上、送信中でない | MP-03 |
